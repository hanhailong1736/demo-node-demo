          ...
          if (!params.lastDate && !params.startDate) {
            ...
            for (let index = 0; index < data.length; index++) {
              ...
              if (params.lastDate) {
                if (params.startDate) {
                  if (
                    element.name >= params.lastDate &&
                    element.name >= params.startDate &&
                    element.name <= params.endDate
                  ) {
                    list.push(element);
                  }
                } else {
...
const openai = new OpenAI({
  apiKey: "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",
});
...
router.all("/get_image", async (req, res) => {
  let params = req.body;
  if (Object.keys(params).length == 0) {
    params = req.query;
  }
...
router.all("/get_image_answer", async (req, res) => {
  let params = req.body;
  if (Object.keys(params).length == 0) {
    params = req.query;
  }
      ...
      new Result({
        ...
        message:
          req.body.language == "zh"
            ? "请求错误,请使用multipart/form-data格式"
            : "Request error, please use multipart / form data format",
      })
    );
    return;
  }
